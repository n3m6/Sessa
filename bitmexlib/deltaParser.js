...
function replaceStore(context, key, newData) {
  ...
  if (!Array.isArray(context[key])) {
    ...
    context[key] = newData[0]; //eslint-disable-line
  } else {
    context[key] = newData; //eslint-disable-line
  }
  return context[key];
}
...
module.exports = {
   ...
   *
   * Returns new data. We ensure that the object identity check will fail
   * by constructing a new array
  ...
  _delete(context, key, data, keys) {
    return removeFromStore(...arguments);
  },

  _insert(context, key, data, keys) {
    return insertIntoStore(...arguments);
  },
